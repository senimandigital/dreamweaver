...
$debug_to_file = false;
function log_messages($error_message) { global $f, $debug_to_file;
if ($debug_to_file === true) {
  if (!is_resource($f)) { @ini_set('display_errors', 1); @error_reporting(E_ALL); $f = @fopen('log.txt', 'a'); }
  if ( is_resource($f)) { return @fwrite($f, $error_message."\n"); }
 }
return false;
}

function create_error($error_msg = '', $line = -1) {
if  ($error_msg != ''){	return '<ERRORS><ERROR><DESCRIPTION>'.$error_msg.'</DESCRIPTION></ERROR></ERRORS>\n'; }
...
foreach($_POST as $key=>$value) {
 if (strtoupper($key) != 'PASSWORD') { log_messages('$_POST["'.$key.'"] = \''.$value."';"); }
}

// We need these information only once when the test button is hit
if (isset($_POST['opCode']) && $_POST['opCode'] == 'IsOpen' ) {
 log_messages("\nPHP-Version: ".phpversion());     // What PHP version
 log_messages('PHP-OS: '.PHP_OS);		   // What OS System is running on
 log_messages('PHP-SAPI-NAME: '.php_sapi_name());  // How PHP is installed (CGI, ISAPI, FastCGI)	// !!!! Major difference exists on these servers for $_ENV, $_SERVER
 log_messages('PHP-Extensions: '.var_export(get_loaded_extensions(),true)); // MySQL, mbstring modules are installed ?!
}
...
if (isset($_POST['Type']) && $_POST['Type'] == 'MYSQL') { require("./mysql.php");
...
// Process opCode
if (isset($oConn) && $oConn) { $oConn->Open();
 if     ($_POST['opCode'] == 'IsOpen'){ $answer = $oConn->TestOpen(); }
 ...
 $oConn->Close();
} else { $answer = create_error('The Connection Module was not initialized properly for an unknown reason.'); }
