...
var SB_FILE = dw.getConfigurationPath() + "/ServerBehaviors/PHP_MySQL/" + SB_NAME;
...
function setDefaultValue(columnValueNode, valueList, nodeList)
{
  ...
  for (var i = 0; !foundDefaultValue && i < nodeList.length; ++i)
  {
    ...
    if (name && columnName.toUpperCase() == name.toUpperCase())
    {
...
function SBInsertRecord_queueDocEdits()
{
  ...
  if (this.bInEditMode)
  {
    ...
    if (!oldFormName || (oldFormName.toUpperCase() != newFormName.toUpperCase()))
    {
  ...
  extPart.queueDocEdits(this.getServerBehaviorFileName(), "Insert Record MySQLi Procedural_main", this.getParameters(), this);
}
